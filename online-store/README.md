## Описание проекта
Создание системы интернет-магазина на языке программирования С++ с использованием принципов
объектно-ориентированного программирования, умных указателей и работы
с PostgreSQL. Система имеет три роли пользователя: администратор, менеджер и покупатель, каждый их них имеет разные уровни доступа к базе данных и имеют разные функции (то что они могут делать).

## Архитектура проекта

1) DatabaseConnection<T> (Шаблонный класс)
Назначение: Управление подключением к PostgreSQL через libpqxx
Ключевые методы: 
-executeQuery() - выполнение SELECT-запросов
-executeNonQuery() - выполнение INSERT/UPDATE/DELETE
-beginTransaction(), commitTransaction(), rollbackTransaction() - работа с транзакциями
-createFunction(), createTrigger() - создание функция и триггеров

2) Абстрактный класс пользователей User
   User (абстрактный класс)
   ├── Admin
   ├── Manager
   └── Customer
В данном классе реализовано наследованое и полиморфизм
User: Базовый класс с чисто виртуальной функцией showMenu(), классы-наследники реализовывают эту функцию по-своему
Admin: Полный доступ к системе
Manager: Управление заказами и складом
Customer: Создание и оплата заказов
3) Классы для работы с заказами
Order: Основной класс заказа
OrderItem: Элементы заказа (композиция с Order)
Payment: Управление оплатой
PaymentStrategy: Стратегия оплаты (в данном случае карта, сбп и электронный кошелек)
## Принципы ООП
1) Наследование
Admin, Manager, Customer - классы наследники от User
2) Полиморфизм
В классе User virtual void showMenu() = 0; - Чисто виртуальная функция, а в классе Admin, Manager, Customer реализована эта функция: void showMenu() override; 
3) Композиция реализована в классе Order
   class Order {
   vector<unique_ptr<OrderItem>> items; // Композиция
   unique_ptr<Payment> payment;         // Композиция
   };
4) Агрегация в классе User
   class User {
   DatabaseConnection<string>* db; // Агрегация
...
1. users - пользователи системы
   ...
   -role (VARCHAR: 'admin', 'manager', 'customer')
   -password_hash (VARCHAR)
   -loyalty_level (INTEGER: 0 — обычный, 1 — премиум)
...
3. orders – заказы
   ...
   -user_id (INTEGER, FOREIGN KEY, ссылается на users)
   -status (VARCHAR: 'pending', 'completed', 'canceled', 'returned')
   -total_price (DECIMAL)
   -order_date (TIMESTAMP)
4. order_items - элементы заказов
   -order_item_id (INTEGER, PRIMARY KEY)
   -order_id (INTEGER, FOREIGN KEY, ссылается на orders)
   ...
   -changed_by (INTEGER, FOREIGN KEY → users)
...
-- Создание функции обновления статуса
...
-- Функция проверки возможности возврата
...
## Триггеры
-Обновление даты заказа при изменении статуса
-Запись истории статусов в order_status_history
...
unique_ptr<OrderItem> item;

// shared_ptr для разделяемого владения
vector<shared_ptr<Order>> orders;

Алгоритмы STL и лямбда-выражения
...
OrderID,UserID,Status,TotalPrice,OrderDate,OldStatus,NewStatus,ChangedAt,ChangedBy,Operation,PerformedAt
1,3,completed,50000.00,2026-01-12 17:10:51.32913,pending,completed,2026-01-12 16:08:43.105583,1,,
1,3,completed,50000.00,2026-01-12 17:10:51.32913,completed,returned,2026-01-12 17:07:29.420137,3,,
1,3,completed,50000.00,2026-01-12 17:10:51.32913,returned,completed,2026-01-12 17:10:51.32913,1,,
...
createdb online_store3

- Выполнение SQL-скриптов
psql -d online_store3 -f sql/database_setup.sql
...
Пример транзацкии
// Добавление продукта с транзакцией
...
cout << "Продукт добавлен успешно" << endl;
}
